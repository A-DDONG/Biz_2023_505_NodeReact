...
// initData.js 에서 export 한 함수들 주엥서 initData() 함수만 필요하니 구조분해를 통하여 import 하기
import { initData } from "../data/initData";
// react-uuid 의 export 타입이 무엇일까
...
const TodoMain = () => {
   ...
   * React 에서는 형제간에 state 를 절대 공유할 수 없다
   * React 에서는 부모가 만들어서 전달해준 State 만 볼 수 있다
   * 자식이 만든 State 는 부모도 볼 수 없다
   *
   * 이 상황을 해결하기 위하여 자식 Comps 에 있던 state 와 state 함수를 부모 Comps 인 TodoMain 으로 끌어 올리기를 한다
   * 그리고, 자식 Comps 에 전달해주어야 한다
   */

  // initData() 함수를 실행하여 initData() 함수가 생성한(return한) 객체로 todo 를 초기화
  ...
  const itemDelete = (id) => {
    // id 에 해당하는 데이터 삭제
    if (window.confirm("정말 삭제함?")) {
      // list 를 forEach 하면서 item 의 id 와 일치하는 데이터가 있으면 해당 데이터를 제외하면서 새로운 리스트 만들기
  ...
  const todoInput = (content) => {
    ...
    // add 또는 Update 를 실행 후 Todo 를 초기화하기
    // setTodo(initData());
  };
