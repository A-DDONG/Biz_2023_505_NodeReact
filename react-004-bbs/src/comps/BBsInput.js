...
const BBsInput = () => {
   ...
   *
   * 만약 유사배열을 새로운 forEach()로 사용하기 위해서는 진짜배열로 변환해 주어야 한다
   * const 진짜배열 =  Array.from(유사배열) : 유사배열을 진짜배열로 변환
   *
   */
  const filesChangeHandler = (e) => {
    ...
    Array.from(files).forEach((file) => {
      ...
      fileReader.onload = (fe) => {
        ...
        // 그래서 setState() 에 데이터를 직접 주입하지 않고 Callback 함수 방식으로 주입한다
