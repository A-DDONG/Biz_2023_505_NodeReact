...
// multer 를 사용하여 파일을 수신할때 multer 가 파일이름을 latin1 방식으로 파일이름을 encoding 해버린다
// windows 환경에서는 한글이름 파일이 깨진다. 한글이름을 UTF-8 로 변환시켜주는 함수
const encKor = (str) => {
  console.log(str);
...
const storageOption = {
  filename: (req, file, cb) => {
    // 전송받은 파일을 encKor() 함수를 통하여 UTF-8 로 변환시키기
...
router.get("/list", async (req, res) => {
  // include
  // sequelize 에서 1:N 관계까 설정되어 있을때 자동 JOIN 하는 코드
...
// localhost:3000/bbs/detail/3 으로 요청이 되면 3이라는 값이 seq 변수에 담기게 된다
// ?seq=값 => queryString 방식
//          req.query.seq 로 값 받기
// /:seq => PathVarriable 방식
//          req.params.seq 로 받기
// form 으로 전송한 데이터는 req.body 에 담겨서 통째로
