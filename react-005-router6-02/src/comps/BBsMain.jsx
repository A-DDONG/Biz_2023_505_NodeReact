...
const BBsMain = () => {
  ...
  // 화면이 최초 렌디렁된 후(Mount 된 후) 에 한번만 실행하라
  // useEffect(() => {}, []);
  useEffect(() => {
  ...
  const bbsRouter = useRoutes([
    {
      ...
      element: <BBsBody />,
      children: [
        // path 에 "" 이 연결된 경우 rootPath 와 함께 제일먼저 보여질 Component
        {
          path: "",
          element: (
            <>
               ...
               * 다. 컴포넌트 사이에 주입된 {bbsListItemView} 컴포넌트는 BBsList 에서 {children} 이라는 정해진 props 변수로 받는다
               */}
        ...
        // to 에 지정한 URL path 로 무조건 화면 전환하기
