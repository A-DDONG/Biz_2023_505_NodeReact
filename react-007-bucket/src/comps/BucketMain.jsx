...
/**
 * 다른 tag 로 감싸진 a tag의 속성 지정하기
 * a tag 는 다른 tag 와 달리 상당히 독자적인 성질을 많이 갖는다
 * 다른 tag(여기에서는 li tag)로 감싸는 구조일 경우
 * 실제 a tag 가 지정된 문자열을 클릭하면 a tag 에 의해 URL 이 변화되는데
 * 만약 a tag 바깥쪽을 클릭하면 실제로 URL 변화등이 없다
 *      <li><a href="#">text</a></li>
 * 위 구조에서 <a href="#"><li>text</li></a> 처럼 만들 수 있지만 이는 HTML5 문법 구조에 어긋나는 패턴이다
 * 이럴때는 a tag 의 display 를 inline-block 으로 설정하고, width, padding 등을 설정하여 li tag 에 가득 채워지도록 만드는 것이 좋다
 */
const UL = styled.ul`
  margin: 10px;
  list-style: none;
...
export const mainLoader = async () => {
  ...
  // return { bucketList : bucketList}; 이름이 같을경우 생략가능
  return { bucketList };
};
